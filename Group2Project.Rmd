...
```{r}
all=regsubsets(quantitative.house$Price~.,data=quantitative.house[2:26])
ShowSubsets(all)
```
...
This transformation allows us to improve our Adjusted R-squared from 0.8516 to 0.8599.
```{r}
...
summary(old.mod.two)
mod.two = lm(Price ~ log(sqrt(LotArea)) + Quality + log(2010 - YearBuilt) + log(2011 - YearRemodel) 
                 + BasementFinSF + BasementSF + GroundSF + GarageSF, data=quantitative.house)
summary(mod.two)
```
...
This transformation allows us to improve our Adjusted R-squared from 0.8611 to 0.8715
```{r}
...
summary(old.mod)
mod = lm(formula = quantitative.house$Price ~ Quality + GroundSF + 
